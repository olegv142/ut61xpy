...
Typically all multimeters have ranges with upper limits separated by a decade. Old multimeters have corresponding shunt resistances separated by decade as well. So if you hit the upper limit of the current range you may switch to the next range where the shunt resistance and so the voltage becomes 10 times smaller. The modern DMM simplifies this schema by grouping sub-ranges into the modes (µA, mA, A) each with its own shunt. Each mode typically have two sub-ranges so that the shunt resistances are separated by two decades. If you hit the upper limit of the smaller sub-range, then you may switch to upper sub-range but this does not change the shunt voltage since both sub-ranges are using the same shunt. If the shunt voltage becomes too high, then the only option you have is to reduce it by the factor of 100 by switching to the upper mode, for example from mA to A. Such switch will reduce the measured counts value by the factor of 100 in the worst case. Therefore if your multimeter have 6000 counts, then you will have only 60 counts in the worst case. The is the direct consequence of having only one shunt per mode. Note that for any other measuring mode but current the worst case count is 10 times smaller than the full scale. So other modes are less demanding regarding the total number of counts.
...
3. UT60BT is absolute outsider despite having 1µV resolution due to PTC fuse which increases its resistance in mA mode up to 3.3kOhm.
