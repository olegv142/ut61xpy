...
If you don't need exceptional precision, then all multimeters with 6000 counts and more will give you reasonably good results in any mode except for the current measuring. Here we will consider why the current measuring mode is so demanding and why popular UNI-T multimeters are actually the least usable models for this task.

### Why measuring current is the most demanding task
The multimeter does not measure current directly. It does measure voltage across current sensing resistor aka *shunt*. This voltage is effectively subtracted from the voltage in the net where you are measuring current. Suppose you are interesting in the current being consumed by microcontroller powered from 3.3V. If the voltage across the shunt is 0.5V then only 2.7V is left for microcontroller. This may affect it seriously reducing the current or even prevent microcontroller from working normally. Therefore, it is generally not desirable for the shunt voltage to exceed 100mV, which can be considered safe and should keep the current disturbance level at 3% (0.1/3.3). In practice many multimeters have full scale voltage across their shunts larger than that. For comparison I have made a [table](https://github.com/olegv142/ut61xpy/blob/main/misc/current_ranges.md) listing shunt resistance and full scale voltage for various multimeters including ones without PC interface, with manual ranges and even old analog multimeters with moving physical needle as display device.

### Why 6000 counts is not enough for measuring current
Typically all multimeters have ranges with upper limits separated by a decade. Old multimeters have corresponding shunt resistances separated by decade as well. So if you hit the upper limit of the current range you may switch to the next range where the shunt resistance and so the voltage becomes 10 times smaller. The modern DMM simplify this schema by grouping sub-ranges into the modes (µA, mA, A) each with its own shunt. Each mode typically have two sub-ranges so that the shunt resistances are separated by two decades. If you hit the upper limit of the smaller sub-range, then you may switch to upper sub-range but this does not change the shunt voltage since both sub-ranges are using the same shunt. If the shunt voltage becomes too high, then the only option you have is to reduce it by the factor of 100 by switching to the upper mode, for example from mA to A. Such switch will reduce the measured counts value by the factor of 100 in the worst case. Therefore if your multimeter have 6000 counts, then you will have only 60 counts in the worst case. This is the direct consequence of having only one shunt per mode. Note that for any other measuring mode but current the worst case count is 10 times smaller than the full scale. So other modes are less demanding regarding the total number of counts.

### Why 1µV resolution is necessary for measuring current
Each multimeter may be characterized by the shunt voltage measuring resolution. Suppose the DMM has this resolution equal to 10µV as most 6000 counts models typically have. This means that given the shunt voltage limit of 100mV you have at most 10000 counts regardless of the total counts of your multimeter. But if you hit the 100mV limit and switch to the upper mode, then you will have 100 times smaller number of counts. That is with 10µV resolution you will have only 100 counts in the worst case. That's why for the best current measuring accuracy without rising shunt voltage to unacceptable level you need 1µV resolution. It will give you 1000 counts with 100mV shunt voltage in the worst case.
...
2. The popular UT61E+ is bad for current measurements due to the combination of several factors:
 ...
 - high shunt resistance - 5 Ohm in mA mode (vs 1 Ohm for most other models) which will typically force you to choose A mode for current exceeding 20mA
 - dead band in A mode from -5mA to +5mA makes measuring mA current very uncomfortable
3. UT60BT is absolute outsider despite having 1µV resolution due to PTC fuse which increases its resistance in mA mode up to 3.3kOhm. It also have dead band in A mode from -5mA to +5mA and from -10 µV to +10 in voltage mode. Therefore I would say its the least usable device of all.
